...
def beta_test(z, values):
    ...
    for i in range(len(z_beta)):
        ...
        if Ln <= 0:
            ...
            alpha_operator = (z[i] * rho_g)
        elif Ln >= 1:
            beta_operator = (z[i] * ((1 ** n1) / mu_L)) / ((1 ** n1) / mu_L)
            alpha_operator = (z[i] * rho_L)
        else:
...
def simulate_comp_impl(nb, Theta_ref, NT, z):
    ...
    for t in range(NT):
        ...
        for n in range(100):
            ...
            beta_L_deriv2 = beta_L_deriv[:C_interpol]
            alpha_n = beta_L_deriv[C_interpol:]
            ...
            for j in range(1,nb):
                ...
                alpha_n = np.array(alpha_n, copy=True)[C:]

                beta_L_deriv2, beta_deriv2 = beta_L_deriv[:C_interpol], beta_deriv[:C_interpol]
                alpha_deriv = beta_deriv[C_interpol:]

                beta_L_deriv2, beta_deriv2 = Theta * beta_L_deriv2, Theta * beta_deriv2

                for q in range(C):
                    #beta_deriv2[q * C + q] += alpha_deriv[q] # Alpha operator
                    beta_deriv2[q * C + q] += 1
            ...
            #z = [[z_dz[x * C], z_dz[x * C + 1], z_dz[x*C+2]] for x in range(nb)]
...
print('elapsed = ',time.time() - duration)
#z1_plot = z_plot[0]
#z2_plot = z_plot[1]
z3_plot = np.zeros(nb)
for i in range(nb):
    z3_plot[i] = 1 - sum(z_plot[i])
plt.plot(x, z_plot, label='z')
plt.plot(x,z3_plot)
